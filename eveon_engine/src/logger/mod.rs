...
const LEVELS: [Level; Level::Trace as usize] = [
    Level::Error,
    Level::Warn,
    Level::Info,
    Level::Debug,
...
pub fn init_engine() {
    log::set_logger(&EngineLogger).unwrap();
...
impl LoggerUi {
    pub fn ui(&mut self, ui: &mut egui::Ui, state: &mut egui_glfw_gl::EguiInputState) {
        ...
        ui.horizontal(|ui| {
            ...
            ui.with_layout(egui::Layout::right_to_left(egui::Align::Center), |ui| {
                if ui.button("Copy").clicked() {
                    egui_glfw_gl::copy_to_clipboard(state, self.copy_text.clone())
                }
            });
        });
...
pub fn logger_ui(ui: &mut egui::Ui, state: &mut egui_glfw_gl::EguiInputState) {
    let mut logger_ui = LOGGER_UI.lock().unwrap();

    logger_ui.ui(ui, state);
}
